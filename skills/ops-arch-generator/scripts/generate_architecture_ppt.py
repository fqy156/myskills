...
def parse_resource_sheet(rows: list[list[str]]) -> list[dict]:
    if is_standard_resource_sheet(rows):
        return parse_standard_resource_sheet(rows)
...
def is_standard_resource_sheet(rows: list[list[str]]) -> bool:
    return matrix_contains(rows, ["环境", "服务组名称", "服务器名称", "服务器IP"])


def parse_standard_resource_sheet(rows: list[list[str]]) -> list[dict]:
    resources = []
    if len(rows) < 4:
        return resources

    header_top = rows[1]
    header_bottom = rows[2]
    width = max(len(header_top), len(header_bottom))
    header_options: list[list[str]] = []
    for index in range(width):
        top = get_cell(header_top, index)
        bottom = get_cell(header_bottom, index)
        options = []
        if top and bottom:
            options.append(f"{top}{bottom}")
        if bottom:
            options.append(bottom)
        if top:
            options.append(top)
        header_options.append([canonical_text(option) for option in options if option])

    def find_index(*candidates: str) -> int:
        canonical_candidates = [canonical_text(candidate) for candidate in candidates if candidate]
        for index, options in enumerate(header_options):
            for candidate in canonical_candidates:
                if any(candidate == option or candidate in option or option in candidate for option in options):
                    return index
        return -1

    env_idx = find_index("环境")
    purpose_idx = find_index("服务组名称", "服务组名称(下拉框没有的选项可自行填写)")
    type_idx = find_index("成本类型")
    name_idx = find_index("服务器名称")
    ip_idx = find_index("服务器IP")
    vip_idx = find_index("VIP")
    cpu_idx = find_index("CPU")
    memory_idx = find_index("内存(G)", "内存（G)")
    system_disk_idx = find_index("系统盘")
    data_disk_idx = find_index("数据盘")
    mount_dir_idx = find_index("数据盘挂载目录")
    disk_type_idx = find_index("类型")
    os_version_idx = find_index("操作系统")
    ...
    for row in rows[3:]:
        ...
        purpose = get_cell(row, purpose_idx)
        ...
        record = {
            "env": current_env,
            "purpose": current_purpose,
            "resource_type": resource_type or current_type,
            "name": get_cell(row, name_idx) or current_purpose,
            "os_version": get_cell(row, os_version_idx),
            "ip": get_cell(row, ip_idx),
            "vip": get_cell(row, vip_idx),
            "lb_ip": "",
            ...
            "mount_dir": get_cell(row, mount_dir_idx),
            "disk_type": get_cell(row, disk_type_idx),
        }
        if canonical_text(record["env"]).startswith("总计"):
            continue
        meaningful_fields = (
            record["purpose"],
            record["name"],
...
def build_header_options(primary_row: list[str], secondary_row: list[str] | None = None) -> list[list[str]]:
    secondary_row = secondary_row or []
    width = max(len(primary_row), len(secondary_row))
    header_options: list[list[str]] = []
    for index in range(width):
        top = get_cell(primary_row, index)
        bottom = get_cell(secondary_row, index)
        options = []
        if top and bottom:
            options.append(f"{top}{bottom}")
        if bottom:
...
def find_header_index(header_options: list[list[str]], *candidates: str) -> int:
    canonical_candidates = [canonical_text(candidate) for candidate in candidates if candidate]
    for index, options in enumerate(header_options):
        for candidate in canonical_candidates:
            if any(candidate == option or candidate in option for option in options):
                return index
    return -1


def looks_like_pod_sheet(sheet_name: str, rows: list[list[str]]) -> bool:
    if not rows:
        return False
    header_text = canonical_text(" ".join(" ".join(row) for row in rows[:8]))
    sheet_key = canonical_text(sheet_name)
    keyword_hits = sum(
        1
        for keyword in (
            "容器节点名称",
        ...
        if canonical_text(keyword) in header_text
    )
    has_identity = any(canonical_text(keyword) in header_text for keyword in ("容器节点名称", "服务描述", "POD资源配置"))
    has_capacity = any(canonical_text(keyword) in header_text for keyword in ("副本数", "小计资源", "JVM堆内存配置(G)"))
    has_runtime = any(canonical_text(keyword) in header_text for keyword in ("端口映射", "容器端口", "外部端口", "容器内路径", "宿主机路径"))
    name_hint = any(token in sheet_key for token in (canonical_text("容器"), "pod"))
    return (has_identity and has_capacity and keyword_hits >= 3) or (name_hint and has_identity and keyword_hits >= 2) or (has_identity and has_capacity and has_runtime)


def locate_pod_header_rows(rows: list[list[str]]) -> tuple[int, int]:
    search_limit = min(len(rows), 8)
    for index in range(search_limit):
        row_text = canonical_text(" ".join(rows[index]))
        next_text = canonical_text(" ".join(rows[index + 1])) if index + 1 < len(rows) else ""
        if any(token in row_text for token in (canonical_text("容器节点名称"), canonical_text("POD资源配置"), canonical_text("服务描述"))):
            if any(
                token in next_text
                for token in (
                    canonical_text("容器端口"),
                    canonical_text("外部端口"),
                    canonical_text("容器内路径"),
                    canonical_text("宿主机路径"),
                    canonical_text("vCPU(核)"),
                    canonical_text("内存(G)"),
                )
            ):
...
def parse_pod_sheet(rows: list[list[str]]) -> list[dict]:
    pods = []
    current_env = ""
    header_top_idx, header_bottom_idx = locate_pod_header_rows(rows)
    header_options = build_header_options(rows[header_top_idx], rows[header_bottom_idx] if header_bottom_idx != header_top_idx else None)

    env_idx = find_header_index(header_options, "环境", "namespace")
    name_idx = find_header_index(header_options, "容器节点名称", "Pod名称", "服务名称")
    description_idx = find_header_index(header_options, "服务描述", "描述")
    cpu_idx = find_header_index(header_options, "POD资源配置vCPU(核)", "vCPU(核)", "CPU")
    memory_idx = find_header_index(header_options, "POD资源配置内存(G)", "内存(G)", "内存（G)")
    replicas_idx = find_header_index(header_options, "副本数", "副本")
    total_cpu_idx = find_header_index(header_options, "小计资源vCPU(核)", "小计资源CPU", "总CPU")
    total_memory_idx = find_header_index(header_options, "小计资源内存(G)", "总内存")
    jvm_heap_idx = find_header_index(header_options, "JVM堆内存配置(G)")
    container_port_idx = find_header_index(header_options, "端口映射容器端口", "容器端口")
    external_port_idx = find_header_index(header_options, "端口映射外部端口", "外部端口")
    container_path_idx = find_header_index(header_options, "磁盘挂载容器内路径", "容器内路径")
    host_path_idx = find_header_index(header_options, "磁盘挂载宿主机路径(落盘在共享存储)", "宿主机路径")
    domain_idx = find_header_index(header_options, "domain", "域名")
    appids_idx = find_header_index(header_options, "appids", "appid")
    libs_idx = find_header_index(header_options, "libs", "lib")

    data_start_idx = header_bottom_idx + 1
    for row in rows[data_start_idx:]:
        ...
        if not name:
            continue
        if any(marker in name for marker in ("资源合计", "合计")):
            break
        pods.append(
            {
                "env": get_cell(row, env_idx) or current_env,
                "name": name,
                "description": get_cell(row, description_idx),
                "cpu": get_cell(row, cpu_idx),
                "memory": get_cell(row, memory_idx),
                "replicas": get_cell(row, replicas_idx),
                "total_cpu": get_cell(row, total_cpu_idx),
                "total_memory": get_cell(row, total_memory_idx),
                "jvm_heap": get_cell(row, jvm_heap_idx),
                "container_port": get_cell(row, container_port_idx),
                "external_port": get_cell(row, external_port_idx),
                "container_path": get_cell(row, container_path_idx),
                "host_path": get_cell(row, host_path_idx),
                "domain": get_cell(row, domain_idx),
                "appids": get_cell(row, appids_idx),
                "libs": get_cell(row, libs_idx),
            }
        )
    return pods
...
def identify_sheet_roles(reader: XlsxReader) -> dict[str, str]:
    ...
    for sheet_name in reader.sheets:
        ...
        if "resource" not in roles and (
            ...
            or is_standard_resource_sheet(rows)
        ):
            roles["resource"] = sheet_name
            continue
        if "pod" not in roles and looks_like_pod_sheet(sheet_name, rows):
...
def build_slide_package_rels_xml(slide_number: int) -> bytes:
    ns_uri = "http://schemas.openxmlformats.org/package/2006/relationships"
    root = ET.Element(f"{{{ns_uri}}}Relationships")
    ET.SubElement(
        root,
        f"{{{ns_uri}}}Relationship",
        {
            ...
            "Target": "../slideLayouts/slideLayout26.xml",
        },
    )
    if slide_number == 1:
        ET.SubElement(
...
def build_presentation_xml(existing_xml: bytes, slide_count: int) -> bytes:
    ns = {
    ...
    ET.register_namespace("p14", ns["p14"])
    ET.register_namespace("p15", "http://schemas.microsoft.com/office/powerpoint/2012/main")
    root = ET.fromstring(existing_xml)
    existing_slide_rids = []
    sld_id_list_existing = root.find("p:sldIdLst", ns)
    for child in (list(sld_id_list_existing) if sld_id_list_existing is not None else []):
        rid = child.attrib.get(f"{{{ns['r']}}}id")
        if rid:
            existing_slide_rids.append(rid)

    def next_slide_rid(index: int) -> str:
        if index < len(existing_slide_rids):
            return existing_slide_rids[index]
        return f"rIdSlide{index + 1}"
    ...
    if sld_id_list is not None:
        ...
        for index in range(slide_count):
            slide_id = str(2147481810 + index)
            slide_rid = next_slide_rid(index)
            ET.SubElement(
                sld_id_list,
                f"{{{ns['p']}}}sldId",
                {
    ...
    section_refs = root.findall(".//p14:section/p14:sldIdLst", ns)
    for section_list in section_refs:
        for child in list(section_list):
            section_list.remove(child)
        for index in range(slide_count):
            ET.SubElement(section_list, f"{{{ns['p14']}}}sldId", {"id": str(2147481810 + index)})

    return ET.tostring(root, encoding="utf-8", xml_declaration=True)


def build_presentation_rels_xml(existing_xml: bytes, slide_count: int) -> bytes:
    ns_uri = "http://schemas.openxmlformats.org/package/2006/relationships"
    ...
    for child in existing:
        if child.attrib.get("Type") == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide":
            existing_slide_rids.append(child.attrib.get("Id"))
            root.remove(child)

    def next_slide_rid(index: int) -> str:
        if index < len(existing_slide_rids) and existing_slide_rids[index]:
            return existing_slide_rids[index]
        return f"rIdSlide{index + 1}"
...
def build_content_types_xml(existing_xml: bytes, slide_count: int) -> bytes:
    ns_uri = "http://schemas.openxmlformats.org/package/2006/content-types"
    ET.register_namespace("", ns_uri)
    root = ET.fromstring(existing_xml)
    existing = list(root)
    for child in existing:
        if child.tag == f"{{{ns_uri}}}Override" and child.attrib.get("PartName", "").startswith("/ppt/slides/slide"):
            root.remove(child)
...
def collect_extra_media(slides: list[dict]) -> dict[str, bytes]:
    extra_media = {}
    for slide in slides:
        for _, target in slide.get("image_rels", []):
...
def write_pptx_from_template(template_path: Path, output_path: Path, slides: list[dict]) -> None:
    extra_media = collect_extra_media(slides)
    ...
    with zipfile.ZipFile(template_path, "r") as source, zipfile.ZipFile(tmp_output, "w", compression=zipfile.ZIP_DEFLATED) as target:
        ...
        for info in source.infolist():
            if info.filename.startswith("ppt/slides/slide") and info.filename != "ppt/slides/slide1.xml":
                continue
            if info.filename.startswith("ppt/slides/_rels/slide") and info.filename != "ppt/slides/_rels/slide1.xml.rels":
                continue
            if info.filename == "ppt/slides/slide1.xml":
                data = slides[0]["slide_xml"].encode("utf-8")
            elif info.filename == "ppt/slides/_rels/slide1.xml.rels":
                base_rels = build_slide_package_rels_xml(1)
                data = build_slide_rels_xml(base_rels, slides[0].get("image_rels", []))
            elif info.filename == "ppt/presentation.xml":
                data = build_presentation_xml(source.read(info.filename), len(slides))
            elif info.filename == "ppt/_rels/presentation.xml.rels":
                data = build_presentation_rels_xml(source.read(info.filename), len(slides))
            elif info.filename == "[Content_Types].xml":
                data = build_content_types_xml(source.read(info.filename), len(slides))
            else:
        ...
        for slide_number, slide in enumerate(slides[1:], start=2):
            ...
            rels_xml_name = f"ppt/slides/_rels/slide{slide_number}.xml.rels"
            target.writestr(slide_xml_name, slide["slide_xml"].encode("utf-8"))
            slide_rels_xml = build_slide_rels_xml(build_slide_package_rels_xml(slide_number), slide.get("image_rels", []))
            target.writestr(rels_xml_name, slide_rels_xml)
...
def validate_pptx(path: Path) -> dict:
    ...
    return {"testzip": broken or "", "slide_xml_valid": True, "slides": len(slide_parts)}
...
def group_records_by_env(resources: list[dict], pods: list[dict]) -> list[tuple[str, list[dict], list[dict]]]:
    order = []
    grouped_resources: dict[str, list[dict]] = defaultdict(list)
    grouped_pods: dict[str, list[dict]] = defaultdict(list)
    resource_envs = []

    def remember_env(env: str) -> None:
        env_key = env or "默认"
        if env_key not in order:
            order.append(env_key)

    for resource in resources:
        env = (resource.get("env") or "").strip() or "默认"
        remember_env(env)
        grouped_resources[env].append(resource)
        if env != "默认" and env not in resource_envs:
            resource_envs.append(env)
    pod_fallback_env = resource_envs[0] if len(resource_envs) == 1 else "默认"
    for pod in pods:
        raw_env = (pod.get("env") or "").strip()
        normalized_env = re.sub(r"[（(].*?[）)]", "", raw_env).strip()
        env = raw_env or pod_fallback_env
        if normalized_env and normalized_env in grouped_resources:
            env = normalized_env
        elif not raw_env:
            env = pod_fallback_env
        elif len(resource_envs) == 1 and normalized_env and normalized_env not in grouped_resources:
            env = pod_fallback_env
        remember_env(env)
        grouped_pods[env].append(pod)

    return [(env, grouped_resources.get(env, []), grouped_pods.get(env, [])) for env in order]


def env_title(env_name: str, default_title: str | None = None) -> str:
    if default_title:
        return default_title
    env_label = env_name or "客户"
    if env_label != "默认" and "环境" not in env_label:
        env_label = f"{env_label}环境"
    return f"{env_label}部署图"
...
def main() -> int:
    ...
    env_groups = group_records_by_env(resources, pods)
    slide_payloads = []
    slide_summaries = []
    all_services = []
    all_unmatched_resources = []
    all_conflicts = []
    all_connections = []
    all_families: dict[str, dict] = {}

    for index, (env_name, env_resources, env_pods) in enumerate(env_groups):
        services = synthesize_services_from_resources(env_resources, env_pods)
        families, unmatched_resources = build_families(env_resources, services, env_pods)
        conflicts = detect_port_conflicts(services, env_pods)
        title = env_title(env_name, args.title if len(env_groups) == 1 else None)
        slide_xml, connections, image_rels = render_diagram(title, families, env_pods, slide_width, slide_height)
        ...
        all_unmatched_resources.extend(unmatched_resources)
        all_conflicts.extend(conflicts)
        all_connections.extend(connections)
        for key, family in families.items():
            all_families[f"{env_name}:{key}:{index}"] = family

    title = args.title or (slide_payloads[0]["title"] if len(slide_payloads) == 1 else f"{workbook_path.stem}部署图")
    ...
    write_pptx_from_template(template_path, output_pptx, slide_payloads)
